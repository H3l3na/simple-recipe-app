...
public class RecipeService {
    ...
    private void mapIngredients(RecipeDTO dto, Long recipeId) {
        // Ensure the ingredients list is never null
        List<Ingredient> ingredients = ingredientRepository.findByRecipeId(recipeId);
        ingredients = (ingredients != null) ? ingredients : Collections.emptyList();

        dto.setIngredients(IngredientMapper.toDTO(ingredients));

        if (!ingredients.isEmpty()) {
            double totalCalories = ingredients.stream()
                    .mapToDouble(ingredient -> ingredient.getNumberOfCalories() != null
                            ? ingredient.getNumberOfCalories()
                            : 0)
                    .sum();

            Integer numberOfServings = dto.getNumberOfServings();
            if (numberOfServings == null || numberOfServings <= 0) {
                numberOfServings = 1; // Fallback to avoid division by zero
            }

            int caloriesPerServing = (int) (totalCalories / numberOfServings);
            dto.setCaloriesPerServing(caloriesPerServing);
        }
    }
